...
struct PracticeScreen: View {
    ...
    @State private var didLogPracticeStart = false
                        ...
                        Button(index == sentences.count - 1 ? "Finish" : "Check") {
                            if checked == true {
                                if index == sentences.count - 1 {
                                    if let content {
                                        AppAnalytics.logPracticeComplete(
                                            storyId: content.id,
                                            storyTitle: content.title,
                                ...
                                let success = selectedTokens.joined(separator: " ") == sentenceTokens.joined(separator: " ")
                                checked = success
                                if let content {
                                    AppAnalytics.logListeningPuzzleEvaluated(
                                        ...
                                        segmentCount: sentenceTokens.count,
                                        tokenCount: selectedTokens.count,
                                        success: success
                                    )
                                }
                            }
    ...
    private func logPracticeStartIfNeeded() {
        guard !didLogPracticeStart, let content else { return }
        didLogPracticeStart = true
